...
public class ProviderProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.btnHire).setOnClickListener(v -> {
            if (selectedItems.isEmpty()) { Toast.makeText(this, "Selecione pelo menos um serviço", Toast.LENGTH_SHORT).show(); return; }
            openPayment();
        });
        ...
        setupSocialActions();
        setupCatalogForProvider();
        setupReviewsForProvider();
    }
    ...
    private void initUI() {
        ...
        if (reviews != null) ((TextView) findViewById(R.id.tvReviewsCount)).setText(reviews + " avaliações");

        TextView tvDesc = findViewById(R.id.tvDescription);
        ImageView ivLogo = findViewById(R.id.ivProviderLogoProfile);
        ...
        if ("Salão Bela Forma".equals(providerName)) {
            ivLogo.setImageResource(R.drawable.logobelaforma); tvInsta.setText("@belaformal"); tvPhone.setText("(11) 98765-4321");
            ...
            ivLogo.setImageResource(R.drawable.logotechfix); tvInsta.setText("@techfix_oficial"); tvPhone.setText("(11) 91234-5678");
            ...
            ivLogo.setImageResource(R.drawable.logopizzarianapolitana); tvInsta.setText("@pizzaria_napoli"); tvPhone.setText("(11) 3344-5566");
            ...
            ivLogo.setImageResource(R.drawable.logofitlife); tvInsta.setText("@fitlife_academia"); tvPhone.setText("(11) 99887-7665");
            ...
            ivLogo.setImageResource(R.drawable.logoautomecanicasilva); tvInsta.setText("@mecanica_silva"); tvPhone.setText("(11) 4002-8922");
            ...
            ivLogo.setImageResource(R.drawable.logoescoladeidiomasglobal); tvInsta.setText("@idiomas_global"); tvPhone.setText("(11) 3214-5678");
    ...
    private void setupCatalogForProvider() {
        ...
        if ("Salão Bela Forma".equals(providerName)) {
            setCatalogItem(0, "Corte Masculino", "R$ 50,00", "30 min"); setCatalogItem(1, "Corte Feminino", "R$ 70,00", "45 min");
            setCatalogItem(2, "Escova", "R$ 40,00", "40 min"); setCatalogItem(3, "Manicure", "R$ 35,00", "30 min"); setCatalogItem(4, "Corte + Escova", "R$ 80,00", "1h 15min");
        } else if ("TechFix Consertos".equals(providerName)) {
            setCatalogItem(0, "Manutenção de Notebook", "R$ 150,00", "1h 30min"); setCatalogItem(1, "Limpeza Preventiva", "R$ 100,00", "1h");
            setCatalogItem(2, "Formatação + Backup", "R$ 150,00", "2h"); setCatalogItem(3, "Upgrade SSD 240GB", "R$ 250,00", "1h"); setCatalogItem(4, "Troca de Tela", "R$ 450,00", "3h");
        } else if ("Pizzaria Napolitana".equals(providerName)) {
            setCatalogItem(0, "Pizza Pequena", "R$ 30,00", "30 min"); setCatalogItem(1, "Pizza Média", "R$ 40,00", "30 min");
            setCatalogItem(2, "Pizza Grande", "R$ 45,00", "35 min"); setCatalogItem(3, "Refrigerante 2L", "R$ 12,00", "5 min"); hideItem(4);
        } else if ("Academia FitLife".equals(providerName)) {
            setCatalogItem(0, "Plano Mensal", "R$ 120,00", "30 dias"); setCatalogItem(1, "Plano Trimestral", "R$ 300,00", "90 dias");
            setCatalogItem(2, "Plano Anual", "R$ 1.000,00", "1 ano"); setCatalogItem(3, "Avaliação Física", "R$ 50,00", "45 min"); hideItem(4);
        } else if ("Auto Mecânica Silva".equals(providerName)) {
            setCatalogItem(0, "Troca de Óleo", "R$ 180,00", "40 min"); setCatalogItem(1, "Alinhamento e Balanceamento", "R$ 120,00", "1h");
            setCatalogItem(2, "Revisão Completa", "R$ 280,00", "3h"); setCatalogItem(3, "Limpeza de Bicos", "R$ 150,00", "1h 30min"); hideItem(4);
        } else if ("Escola de Idiomas Global".equals(providerName)) {
            setCatalogItem(0, "Matrícula", "R$ 100,00", "-"); setCatalogItem(1, "Mensalidade Inglês", "R$ 200,00", "Mensal");
            setCatalogItem(2, "Material Didático", "R$ 150,00", "-"); setCatalogItem(3, "Aula Experimental", "R$ 30,00", "1h"); hideItem(4);
        }
        for (int id : itemLayouts) {
            View v = findViewById(id);
            if (v == null) continue;
            v.setOnClickListener(view -> {
                if (selectedItems.contains(id)) { selectedItems.remove(id); view.setBackgroundResource(R.drawable.bg_payment_normal); }
                else { selectedItems.add(id); view.setBackgroundResource(R.drawable.bg_payment_selected); }
            });
        }
    }
    ...
    private void setCatalogItem(int index, String name, String price, String duration) {
        ...
        ((TextView) findViewById(nameIds[index])).setText(name); ((TextView) findViewById(priceIds[index])).setText(price);
        ((TextView) findViewById(durationIds[index])).setText(duration); findViewById(layoutIds[index]).setVisibility(View.VISIBLE);
    }

    private void hideItem(int index) { int[] layoutIds = {R.id.itemCatalog1, R.id.itemCatalog2, R.id.itemCatalog3, R.id.itemCatalog4, R.id.itemCatalog5}; findViewById(layoutIds[index]).setVisibility(View.GONE); }
    ...
    private void setReview(int index, String avatar, String name, String date, String comment, int stars) {
        ...
        ((TextView) findViewById(avatarId)).setText(avatar); ((TextView) findViewById(nameId)).setText(name);
        ((TextView) findViewById(dateId)).setText(date); ((TextView) findViewById(commentId)).setText(comment);

        LinearLayout starsContainer = findViewById(starsContainerId);
        if (starsContainer != null) {
            for (int i = 0; i < 5; i++) {
                ImageView star = (ImageView) starsContainer.getChildAt(i);
                if (i < stars) { star.setImageResource(R.drawable.ic_star); star.setColorFilter(ContextCompat.getColor(this, R.color.terracota)); }
    ...
    private void openPayment() {
        ...
        ArrayList<String> names = new ArrayList<>(), prices = new ArrayList<>(), durations = new ArrayList<>();
        ...
        intent.putStringArrayListExtra("serviceNames", names); intent.putStringArrayListExtra("servicePrices", prices);
        intent.putStringArrayListExtra("serviceDurations", durations); intent.putExtra("providerName", providerName);
        intent.putExtra("category", category); intent.putExtra("address", address); startActivity(intent);
    }

    private void setupSocialActions() {
        findViewById(R.id.llPhone).setOnClickListener(v -> Toast.makeText(this, "Ligando para o prestador...", Toast.LENGTH_SHORT).show());
        findViewById(R.id.llInstagram).setOnClickListener(v -> Toast.makeText(this, "Abrindo Instagram...", Toast.LENGTH_SHORT).show());
    }
}
